...
class ParentalControlSettingsAdapter(private val context: Context, private val list : ArrayList<SettingsModel>) : RecyclerView.Adapter<ParentalControlSettingsAdapter.ShowsHolder>() {
    ...
    override fun onBindViewHolder(holder: ShowsHolder, position: Int) {
       ...
       holder.text.text = list[position].settingName
        ...
        if (list[position].isHeading) {
            holder.textHeading.visibility = View.VISIBLE
            holder.textHeading.text = list[position].settingName
